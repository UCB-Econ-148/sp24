...
weekdays = {key: value for key, value in
            zip(range(1, 8), ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"])}


class bcolors:
    """
    Colors to use for printed output
    """
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
    OKCYAN = '\033[96m'
...
if __name__ == '__main__':
    ...
    # check if _modules folder exists and contains files; if so raise a warning to the user
    ...
    # back out the date of the first week's monday (used later) 
    first_monday = semester_start_date - timedelta(days=semester_start_date.weekday())
    print()

    # get the days of week for the lectures
    while True:
        try:
    ...
    # check if the semester_start_date is actually one of those days of week
    if semester_start_date.weekday() + 1 not in lecture_days:
        raise ValueError(
            f"{semester_start_date.strftime('%m/%d/%Y')} is a {weekdays[semester_start_date.weekday() + 1]}, which is not in {', '.join([weekdays[day] for day in lecture_days])}{bcolors.ENDC}. ")
    ...
    # get the number of week in the semester
    while True:
        try:
    ...
    # get the dates to skip when generating weekly schedule markdowns
    ...
    # generate weekly schedule markdowns
    count = 1
    os.makedirs("_modules", exist_ok=True)

    # week 1 (the first week needs special handling)
    md_texts = []
    md_texts.append("---")
    ...
    for dow in lecture_days:
        if dow >= semester_start_date.weekday() + 1:
            if (first_monday + timedelta(days=7 * 0 + dow - 1)) not in dates_to_skip:
                md_texts.append(
                    f'      - date: {(first_monday + timedelta(days=7 * 0 + dow - 1)).strftime("%Y-%m-%d")}')
    ...
    for week in range(1, num_weeks):
        ...
        for dow in lecture_days:
            if (first_monday + timedelta(days=7 * week + dow - 1)) not in dates_to_skip:
                md_texts.append(
                    f'      - date: {(first_monday + timedelta(days=7 * week + dow - 1)).strftime("%Y-%m-%d")}')
                md_texts.append(f'        events:')
                md_texts.append(f'          "**{count}**{{: .label .label-gray }} Lecture {count}":')
                md_texts.append(f'            "**{count}**{{: .label .label-ghost }} slides • video"')
                count += 1
